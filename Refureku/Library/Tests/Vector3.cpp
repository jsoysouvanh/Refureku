...
rfk::Archetype const* rfk::getArchetype<Vector3f>() noexcept
{
	...
	if (!initialized)
	{
		...
		//Register fields
		rfk::Field* field;
		field = type.addField("x", stringHasher("some_namespace::Vector3<float>::x"), rfk::getType<float>(), rfk::EFieldFlags::Public, &type, offsetof(Vector3f, x));
		field = type.addField("y", stringHasher("some_namespace::Vector3<float>::y"), rfk::getType<float>(), rfk::EFieldFlags::Public, &type, offsetof(Vector3f, y));
		field = type.addField("z", stringHasher("some_namespace::Vector3<float>::z"), rfk::getType<float>(), rfk::EFieldFlags::Public, &type, offsetof(Vector3f, z));
		...
		/*rfk::StaticField* staticField = */type.addStaticField("exampleStaticField", stringHasher("some_namespace::Vector3<float>::exampleStaticField"), rfk::getType<float>(), rfk::EFieldFlags::Public | rfk::EFieldFlags::Static, &type, &some_namespace::Vector3<float>::exampleStaticField);
		...
		staticMethod->addParameter("lhs", rfk::getType<Vector3f const&>())->addParameter("rhs", rfk::getType<Vector3f const&>());
	}
